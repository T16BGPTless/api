...
# CASE 1: SUCCESS (200 OK) - API key is reset
def test_regiser_API_token_success(client):
    mock_existing = MockResponse(data=[{"group_name": "test-group", "api_token" : "old-token"}])
    mock_update = MockResponse(data=[{"group_name": "test-group", "api_token" : "new-token"}])
    
    payload = {"group_name": "test-group"}
    
    with patch("app.routes.auth.get_db", return_value=MagicMock()), \
         patch("app.routes.auth.sb_execute", side_effect=[mock_existing, mock_update]), \
         patch("app.routes.auth.sb_has_error", return_value=False), \
         patch("app.routes.auth.uuid.uuid4") as mock_uuid:
        
        mock_uuid.return_value.hex = "new-token"
        ...
        assert response.get_json == {"APItoken" : "new-token"}
