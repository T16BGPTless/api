...
def test_delete_invoice_success(client):
    ...
    mock_existing = MockResponse(
        data=[{"owner_token": 10, "xml": mock_xml, "deleted": False}]
    )
    ...
    with (
        ...
        patch(
            ...
            side_effect=[mock_existing, mock_group_lookup, mock_update],
        ),
...
def test_delete_invoice_update_error(client):
    ...
    mock_existing = MockResponse(data=[{"owner_token": 10, "deleted": False}])
    mock_group_lookup = MockResponse(data=[{"id": 10}])
    ...
    with (
        ...
        patch(
            "app.routes.invoices.sb_execute",
            side_effect=[mock_existing, mock_group_lookup, None],
        ),
    ):
